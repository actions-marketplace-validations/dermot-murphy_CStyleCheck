...
[![Naming Convention](https://github.com/dermot-murphy/CStyleCheck/actions/workflows/cstylecheck_rules.yml/badge.svg)](https://github.com/dermot-murphy/CStyleCheck/actions/workflows/cstylecheck_rules.yml)
...
documentation for all 50 rules with YAML configuration and annotated C examples.
...
```
.pre-commit-hooks.yaml   # pre-commit hook definition (id: cstylecheck)
pyproject.toml           # pip / pipx / pre-commit packaging metadata
Rules-and-Configuration.md  # wiki: all 50 rules with config and examples
src/
    cstylecheck.py          # the linter (~3 200 lines, stdlib only)
    cstylecheck_rules.yaml   # project-specific rule configuration
...
tests/
    cstylecheck_rules.yaml   # test-suite config (independent of src/)
...
Dockerfile/
    ...
    .dockerignore
.github/workflows/
    cstylecheck_tests.yml      # runs the test suite on every commit
...
> **Note:** `tests/cstylecheck_rules.yaml` is the configuration used by the test
> suite and is kept independent of `src/cstylecheck_rules.yaml` so that the
...
python src/cstylecheck.py --config src/cstylecheck_rules.yaml \
    source/**/*.c source/**/*.h

python src/cstylecheck.py --options-file src/cstylecheck.options \
    source/**/*.c source/**/*.h
...
| Flag | Purpose |
|---|---|
| `--config FILE` | YAML rule config (default: `cstylecheck_rules.yaml`) |
...
Configure in `cstylecheck_rules.yaml` under `misc.copyright_header`:
...
Configure in `cstylecheck_rules.yaml` under `misc.eof_comment`:
...
- **Independent test configuration** — `tests/cstylecheck_rules.yaml` is kept
  separate from `src/cstylecheck_rules.yaml` so production tuning never breaks
  test assertions.
  ...
  - repo: https://github.com/dermot-murphy/CStyleCheck
    ...
    hooks:
      - id: cstylecheck
        args:
          - --config
          - cstylecheck_rules.yaml    # path to your config inside the repo
```
...
cstylecheck --config cstylecheck_rules.yaml source/**/*.c source/**/*.h
```
...
docker run --rm \
  ...
  --verbose \
  --config /app/cstylecheck_rules.yaml \
...
|---|---|---|
| `config` | YAML rule config file | `src/cstylecheck_rules.yaml` |
...
1. **Test the action locally** using
   ...
   ```bash
   act -W .github/workflows/cstylecheck_rules.yml
   ```
